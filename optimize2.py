...
def calculate():
    ...
    for epoch in range(num_epochs):
        ...
        target1copy = target1.clone()
        target2copy = target2.clone()

        # find the center of the ellipse
        target1 = torch.tensor(c1)
        target2 = torch.tensor(c2)
        prediction1 = torch.mean(prediction1, axis=1)
        prediction2 = torch.mean(prediction2, axis=1)
        print(prediction1)
        print(prediction2)

        # find the width and height of the ellipse
        # center1 = torch.mean(prediction1, axis=1)
        # center2 = torch.mean(prediction2, axis=1)
        ...
        # center1 = torch.tensor(c1)
        # center2 = torch.tensor(c2)

        # prediction1 = torch.norm(prediction1.T[pred_minor1] - center1)
        # prediction2 = torch.norm(prediction2.T[pred_minor2] - center2)

        # prediction1 = torch.norm(prediction1.T[pred_maj1] - center1)
        # prediction2 = torch.norm(prediction2.T[pred_maj2] - center2)

        # print("prediction 1: ", prediction1, " point: ", maj_p1)
        # print("prediction 2: ", prediction2, " point: ", maj_p2)
        
        # plt.plot(prediction1copy[0].detach().numpy(), prediction1copy[1].detach().numpy(), linewidth=2.5, linestyle='--', label='predicted')
        # plt.show()

        # target1 = torch.tensor(w1)
        # target2 = torch.tensor(w2)
